...
public class EnergyContainerUpdater {
    public static void blowup(TileEntity tileEntity, long voltage) {
        ...
        if (ConfigHolder.doExplosions) {
            tileEntity.getWorld().createExplosion(null, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, GTUtility.getTierByVoltage(voltage), true);
        }
    }
    ...
    public static long suggest(TileEntity tile, EnumFacing side, long voltage, long amperage) {
        ...
        if (otherEntity == null || !otherEntity.hasCapability(GregtechCapabilities.CAPABILITY_ENERGY_CONTAINER, otherSide)) {
            return 0;
        }

        IEnergyContainer otherEnergyContainer = otherEntity.getCapability(GregtechCapabilities.CAPABILITY_ENERGY_CONTAINER, otherSide);
